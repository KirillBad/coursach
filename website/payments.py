    ...
    payment = Payment.create(
        {
            ...
            "metadata": {"type": type, "quantity": quantity},
        },
        idempotence_key,
    )
    ...
    try:
        ...
        if notification_object.event == WebhookNotificationEventType.PAYMENT_SUCCEEDED:
            ...
            payment = db.session.scalars(
                db.select(Payments).where(Payments.id == response_object.id)
            )
            ...
            if response_object.metadata.type == "2":
                payment.user.subscription_end = datetime.now() + timedelta(
                    days=int(response_object.metadata.quantity)
                )
